...
def plot(voxel_matrix, save_dir):
    ...
    fig = go.Figure(data=go.Scatter3d(x=x, y=y, z=z, mode='markers', marker=\
                    dict(size=5, symbol='square', color='#ceabb2', line=dict(width=2,color='black'))))
    fig.update_layout()
    fig.show()
...
def plot_frag(vox_pottery, save_dir):
    ...
    colors= ['#ceabb2', '#d05d86', '#7e1b2f', '#c1375b', '#cdc1c3',
             ...
             '#ceabb2', '#d05d86', '#7e1b2f', '#c1375b', '#cdc1c3',
             '#ceabb2', '#d05d86', '#7e1b2f']
    
    datas = []

    for i in range(1, 18):
        voxels = np.array(np.where(vox_pottery == i)).T  # N*3
        x, y, z = voxels[:, 0], voxels[:, 1], voxels[:, 2]
        data = go.Scatter3d(x=x, y=y, z=z, 
                            mode='markers', 
                            marker=dict(size=5, symbol='square', color=colors[i-1], line=dict(width=2,color='black')), 
                            name=f"fragment {i}")
        datas.append(data)
    fig = go.Figure(datas)
    fig.update_layout()
...
def plot_join(vox_1, vox_2):
    ...
    for i in range(1, 18):
        voxels_1 = np.array(np.where(vox_1 == i)).T  # N*3
        voxels_2 = np.array(np.where(vox_2 == i)).T  
        x1, y1, z1 = voxels_1[:, 0], voxels_1[:, 1], voxels_1[:, 2]
        x2, y2, z2 = voxels_2[:, 0], voxels_2[:, 1], voxels_2[:, 2]
        data_1 = go.Scatter3d(x=x1, y=y1, z=z1, mode='markers', marker=\
                    dict(size=5, symbol='square', color=colors[i-1], line=dict(width=2,color='black')))
        data_2 = go.Scatter3d(x=x2, y=y2, z=z2, mode='markers', marker=\
                    dict(size=5, symbol='square', color=colors[i-1], line=dict(width=2,color='black')))
        datas.append(data_1)
        datas.append(data_2)
